...
- [x] **`find watch --once` exits non-zero on trigger failure** · S — `commandFindWatch` prints `fail(...)` per errored trigger and returns 0 regardless. Under cron, which is exactly what `--once` is for, a silently failing finder looks like a healthy one.
