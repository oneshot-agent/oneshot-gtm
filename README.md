...
| `local-registry`    | newly-licensed/newly-enumerated main-street businesses over free public registries — Socrata business-license portals, the NPPES NPI registry, FMCSA's carrier/broker census (published email, no `findEmail`/`verifyEmail` spend), and Socrata health-inspection portals as an operating-status confirmation                     |
