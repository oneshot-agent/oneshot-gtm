...
Workspaces share one thing: `~/.oneshot-gtm-shared/shared.sqlite`, holding shared person identities, workspace memberships, paid lookup caches and contact touches, so the same person is never bought twice and never first-touched by two workspaces within 7 days. [Workspaces](./docs/workspaces.md) covers resolution order, the shared DB, `doctor`'s cross-workspace checks and the dashboard switcher.
