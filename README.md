...
| `local-registry`    | newly-licensed/enumerated main-street businesses over free, keyless public registries (Socrata business licenses, NPPES NPI, FMCSA Company Census, Socrata health inspections) — recent matches route to `new-business`, older ones to `free-pilot`; fmcsa carries a published email on the record, so it skips `findEmail`/`verifyEmail` entirely |
