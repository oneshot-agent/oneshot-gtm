...
| `local-registry`    | newly-licensed/enumerated main-street businesses over free, keyless public registries — Socrata business-license portals, the NPPES NPI registry, FMCSA Company Census, and Socrata health-inspection portals; FMCSA records carry a published email, so that lane skips `findEmail`/`verifyEmail` entirely                       |
