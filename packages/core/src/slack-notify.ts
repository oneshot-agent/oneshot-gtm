 ...
 */
/**
 * A Slack incoming-webhook URL and nothing else: https, the `hooks.slack.com`
 * host, the `/services/` path. Enforced at both save sites (the /setup route
 * and `config slack-webhook`) so the server never POSTs founder data to an
 * arbitrary destination someone typed or pasted into the field.
 */
export function isSlackWebhookUrl(url: string): boolean {
  ...
  return (
    ...
    parsed.pathname.length > "/services/".length
  );
}
