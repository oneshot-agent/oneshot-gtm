...
describe("slack-notify", () => {
  ...
  describe("postDailySendSummaryIfDue", () => {
    // The quiet-day cases use a day no other test writes summary data for:
    // getLedger() is one shared ledger and only the mocks reset between cases.
    it("does nothing when webhook URL is not configured", async () => {
      vi.spyOn(config, "loadConfig").mockReturnValue({
        slackWebhookUrl: null,
      } as any);

      const posted = await postDailySendSummaryIfDue(new Date("2026-07-02T10:00:00Z"));
    ...
    it("stamps watermark even on quiet days", async () => {
      ...
      expect(ledger.getPollWatermark(SLACK_DAILY_SUMMARY_WATERMARK)).toBe("2026-07-01");
    });
...
describe("isSlackWebhookUrl", () => {
  it("accepts only https hooks.slack.com/services/… URLs", () => {
    expect(isSlackWebhookUrl("https://hooks.slack.com/services/T000/B000/xyz")).toBe(true);
    expect(isSlackWebhookUrl("  https://hooks.slack.com/services/T000/B000/xyz ")).toBe(true);
    for (const bad of [
      "http://hooks.slack.com/services/T000/B000/xyz",
