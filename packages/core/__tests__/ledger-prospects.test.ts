...
/**
 * Focused coverage for the module extracted in #632: prospect CRUD,
 * research-backlog queries, dossier merges and stored ICP verdicts, moved
 * out of `ledger.ts` into `ledger-prospects.ts`'s `ProspectStore`.
 *
 * `prospect-research.test.ts` already exercises this behavior end-to-end
 * through the public `Ledger` API — that coverage predates this extraction
 * (#570/#355) and continues to pass unchanged, which is exactly the parity
 * signal the extraction promises. What's new here is exercising
 * `ProspectStore` DIRECTLY, against a raw `Database` handle with no `Ledger`
 * in the loop at all — mirroring the precedent set by `ledger-receipts.ts`'s
 * "is a pure function of a raw Database handle" section. That is only
 * possible once the code is its own module, so it is coverage this
 * extraction adds, not coverage it inherits.
 */
...
describe("ProspectStore is a pure function of a raw Database handle (issue #632)", () => {
  let db: Database;
 ...
 */
describe("ProspectStore backlog eligibility (issue #632 parity)", () => {
  let db: Database;
  let store: ProspectStore;
 ...
 * the same behavior.
 */
describe("ProspectStore.mergeProspectDossierHalf (issue #632 parity)", () => {
  let db: Database;
  let store: ProspectStore;
