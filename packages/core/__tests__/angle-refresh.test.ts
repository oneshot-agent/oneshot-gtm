...
describe("triggerAngleRefresh", () => {
  ...
  it("lets an OUTCOME trigger through the freshness debounce — it is new evidence (#573)", () => {
    ...
    // exists to stop a plain re-trigger must not discard it.
    const calls: Array<{ id: number; outcome?: string }> = [];
    registerAngleRefreshTrigger((id, context) => {
      calls.push({ id, outcome: context?.outcome?.type });
    });
    const id = h.ledger.upsertProspect({ email: "c2@x.dev" });
    h.ledger.setProspectAngle(id, JSON.stringify({ hook: "fresh" }));

    triggerAngleRefresh(id); // plain re-trigger: debounced
    triggerAngleRefresh(id, { outcome: { type: "meeting", label: "booked" } });

    expect(calls).toEqual([{ id, outcome: "meeting" }]);
  });
