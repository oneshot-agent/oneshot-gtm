...
- `card`: plain text, at most 1500 characters, exactly these four sections in this order, each a heading on its own line followed by short lines:
  ...
  - `NEVER` — what this writer visibly avoids that the target emails do not already forbid: pleasantries, flattery, moralising, faking certainty, listicle hooks, urgency, whatever else the samples show. Only what the samples show. Do NOT list em dashes, three-item lists, negative parallelism, exclamation marks, emoji, hedging or hype vocabulary here: the drafts already strip or ban all of those, and repeating them only pads the card. Do not mention greetings, sign-offs or signatures either: the drafts set those by their own rules (a greeting some of the time, the sender's signature always), so a card that forbids them fights the rules and gets overruled. What the samples do with greetings and sign-offs may be reported once in `notes`, never in the card.
...
- Em dashes, three-item lists, negative parallelism, exclamation marks, emoji, hedging and hype vocabulary are banned in the target emails and stripped from every draft, whatever the card says. Never describe any of them under SENTENCES or MOVES as a habit to keep, and never repeat them under NEVER. If the samples lean on one of them, mention it once in `notes` so the writer knows the drafts will do without it.
