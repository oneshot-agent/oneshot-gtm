...
- YOUR EDGE: the substance of the useful thing you can share — one concrete observation the move makes newly relevant to them, NOT a value claim or a feature list. The Offer beat draws from this and nothing else.
...
- Body: 4-6 short sentences, under 100 words. Follow the 4-step shape from \_humanizer.md.
  ...
  - Offer (1 sentence): a substantive peer-level observation tied to the SPECIFIC challenge of their first 90 days (the bottleneck their {previous → new role} transition surfaces, the decision they'll hit in month one), drawn from YOUR EDGE and nothing else — when it holds several `//`-separated angles, pick exactly ONE, the one that fits this move, and never blend two or mention that others exist. Never invent an incident or a number to make it land; if YOUR EDGE is thin, say less. Name the TOPIC, not a doc. NEVER frame as a deliverable you'd mail ("the first-90-days teardown", "the {previous-company} benchmark") — see _humanizer.md → Banned: invented artifacts.
