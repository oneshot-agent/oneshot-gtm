...
export const TRIGGERS: TriggerSpec[] = [
  ...
  },
  {
    ...
    configBrief:
      'Finds recent stargazers of repos you watch and turns them into prospects. Config: `repos` (array of `{repo:"owner/name", rel:"competitor"|"adjacent", label?, repoEdge?}` — tag a repo `competitor` to pitch a switch (→ competitor-switch) or `adjacent` for a complementary intro (→ repo-interest); `label` is the human name, else derived from the repo; `repoEdge` is an OPTIONAL per-repo line on why THAT repo is notable + the respectful bridge to your offer, used by repo-interest as a shared-taste nod that also shapes the pitch — e.g. a privacy-first repo leads with control/auditability, not "we do it for you"), `yourEdge` (REQUIRED. Observations the founder actually made, never a pitch: several `//`-separated angles, each opening with who it fits (e.g. *For a founder selling to clinics —*), then either a lesson (a named failure and what was learned) or an opportunity (what this reader could do that their peers cannot yet, resting on one concrete capability or number, never an unbacked outcome). Optional `firstTouchFormat` (`standard` default, `brief`, or `split` with `firstTouchSplit` 0 to 1) tests a 3-sentence first email against the standard one; results show per arm in the trigger editor. The tool picks ONE per prospect in code; the email never sees the others; fed to whichever play the repo routes to), `sinceDays` (recency window, default 30), `limit`, `maxCostUsd`. Needs `GITHUB_TOKEN` for any volume. STRATEGIST DUTY: pick repos your buyers\' current tools live in; tag the ones you replace as `competitor`, the rest `adjacent`; give each adjacent repo a `repoEdge` so the intro nods to why they chose THAT tool.',
    readiness: (cfg) => {
  ...
  },
  {
    ...
    configBrief:
      "Scans city/county council agendas via the Legistar/Granicus Web API for items matching your keywords, and pitches the meeting body's own published contact. Config: `cities` (city names mapped to a Legistar client — see `_civic-legistar.ts` for the curated list; unmapped cities are skipped and logged), `keywords` (free word-boundary gate applied to agenda item TITLES before any paid call — e.g. ['AI', 'automation', 'software'] — REQUIRED), `sinceDays` (forward-looking window, default 30), `yourEdge` (REQUIRED. Observations the founder actually made, never a pitch: several `//`-separated angles, each opening with who it fits (e.g. *For a founder selling to clinics —*), then either a lesson (a named failure and what was learned) or an opportunity (what this reader could do that their peers cannot yet, resting on one concrete capability or number, never an unbacked outcome). Optional `firstTouchFormat` (`standard` default, `brief`, or `split` with `firstTouchSplit` 0 to 1) tests a 3-sentence first email against the standard one; results show per arm in the trigger editor. The tool picks ONE per prospect in code; the email never sees the others; here each angle fits a kind of agenda item), `limit`, `maxCostUsd`. No API key needed (Legistar is a public, keyless JSON API). STRATEGIST DUTY: keywords must match the VOCABULARY agenda clerks actually use, not marketing language — 'body-worn camera' beats 'law enforcement AI', 'permitting software' beats 'GovTech'. Routes to `civic-pilot`.",
    readiness: (cfg) => {
      const cities = Array.isArray(cfg["cities"]) ? cfg["cities"] : null;
      if (!cities || cities.filter((c) => typeof c === "string" && c.trim()).length === 0) {
