...
Last verified **2026-09-16** · Bun 1.3.13 · OneShot SDK 0.34.0 · **4152 tests / 321 files** · typecheck + oxlint + oxfmt pass (44 lint warnings, 0 errors). Measured in this worktree (`/home/devrunner/code/oneshotgtm/.worktrees/t_e78e07ab`, branch `ai/gtm/issue-663`, round-2 correction) against the clean merge-base (`db63fed`, checked out standalone via `git worktree add` at `/tmp/oneshotgtm-mergebase-r2`) with `bun install --frozen-lockfile` fresh in both: merge-base is 4138 tests / 319 files, typecheck/lint/fmt:check all clean (44 warnings, 0 errors) — a full-suite run there hit one flaky timeout in `demo-seed.test.ts` (`is deterministic`, `Test timed out in 15000ms` under parallel full-suite load) unrelated to this branch's diff, confirmed to pass in isolation (23/23). This branch's own full-suite run passed clean with no failures (320/320 files, 4147/4147 tests). Round-2 adds two more real-Ledger regression tests (`cadence-reply-ledger.test.ts`, now 4 tests total) covering a repeated claim-loss pass (row already triaged by a prior poll) and a concurrent claim-loss pass (another caller's triage still in flight) — both reverted-and-reran to confirm they fail without the fix (`repliesDetected` wrongly 1, not 0) and pass with it.
