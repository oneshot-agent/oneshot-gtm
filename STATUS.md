...
Last verified **2026-08-30** · Bun 1.3.13 · OneShot SDK 0.22.0 · **2245 tests / 172 files** · typecheck + oxlint + oxfmt clean.
...
- **The CLI has limited machine-readable output.** Read-only commands (`doctor`, `identities list`, `domains list`, `workspace list`, `find drain`, `find watch --once`) now support `--json` for scripted callers. Mutation commands and multi-step flows still require the dashboard or `/api/measure/*` routes.
