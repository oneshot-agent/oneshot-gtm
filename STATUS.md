...
| Ingest endpoint `https://telemetry.oneshotagent.com/v1/cli` | ✅ green — Cloud Run (`oneshot-telemetry-prod`) → BigQuery `telemetry.cli_events`; verified end-to-end |
...
| `bun run lint`                 | ✅ 0 warnings, 0 errors (oxlint, 268 files)                  |
| `bun run fmt:check`            | ✅ all files pass oxfmt                                      |
| `bun run test`                 | ✅ 1229/1229 vitest cases passing (95 test files)            |
...
- **Telemetry endpoint live, public benchmarks page not built** — the CLI now sends anonymous opt-out events to `telemetry.oneshotagent.com` (Cloud Run → BigQuery), verified end-to-end. The public benchmarks page that surfaces aggregates from it is still Phase 3.
