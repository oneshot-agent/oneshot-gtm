...
Backfill, in this order per workspace: `find research-queue --status live` (pending and approved rows), `find research-prospects --scope all --refresh` (every prospect with a profile URL: sent, in cadence, completed, replied), then `find synthesize-angles --refresh --scope active` so follow-ups and reply drafts pick up the new dossier. All three are uncapped by default; `--dry-run` shows counts and an estimate, `--max-cost-usd` bounds a rehearsal, `--no-rejudge` and `--no-company` narrow what is bought. Re-runs skip researched rows and hit the cache for the rest.
