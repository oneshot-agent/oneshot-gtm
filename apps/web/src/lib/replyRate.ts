...
/** "3 replied (4.1%)" once the sample is big enough, else "1 replied · rate after 30 sends (12 so far)". */
export function replyLabel(replied: number, sends: number): string {
  if (sends <= 0) return `${replied} replied`;
  if (sends < MIN_SENDS_FOR_RATE) {
    return `${replied} replied · rate after ${MIN_SENDS_FOR_RATE} sends (${sends} so far)`;
  }
